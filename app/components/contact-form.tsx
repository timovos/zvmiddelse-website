...
export default function ContactForm({ inline = false }: { inline?: boolean }) {
  const [state, handleSubmit] = useForm("mkovvvbp");
  const umami = useUmami();

  useEffect(() => {
    if (state.succeeded) {
      umami.event("contact-form-submit");
    }
  ...
  return (
    ...
    >
      <p>
        Vul het onderstaand contactformulier in op onze website of stuur een e-mail naar <a href="mailto:info@zvmiddelse.nl" className="text-blue-700 underline hover:text-blue-900">info@zvmiddelse.nl</a>.
      </p>
      {state.succeeded ? (
